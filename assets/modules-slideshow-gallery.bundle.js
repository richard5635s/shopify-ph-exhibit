...
window['ThemeModule_GallerySlideshow'] = section_id => {
  return {
    ...
    mounted() {
      //set initial block if in theme editor
    ...
    setSlideMediaToEager(slide) {
      if (slide !== undefined) {
        const media = slide.querySelector('.tile-media');
        if (media) {
    ...
    _setIndexFromThemeEditorEvent(target, detail) {
      ...
      if (target_section_id === this.section_id) {
        const block = target;
        const slideIndex = Array.from(block.closest('.slideshow-track').children).indexOf(block);
